...
export class DynamoDBGenericDao {
  static async get ({
    params,
    fields = []
  }: { params: Params, fields?: string[] }): Promise<object> {
  ...
  }: { params: Params, fields?: string[], _items?: object[], stopOnLimit?: boolean }): Promise<object[]> {
  ...
  static async getAll ({ params, list, fields = [] }: { params: Params, list: object[], fields?: string[] }) {
    let idx = 0
  ...
  static async deleteBatch ({ params, list }: { params: Params, list: object[] }): Promise<void> {
    let idx = 0
  ...
  static async deleteAll (params: Params[] = []): Promise<object[]> {
    return Promise.all(
      params.map(async param => this.delete({
        params: param
      }))
    )
  }

  static async putBatch ({ params, list }: { params: Params, list: object[] }): Promise<void> {
    let idx = 0
   ...
   * }>} packs
   */
  static async executeBatch (packs: object[] = []) {
    ...
    if (Array.isArray(newPacks) && newPacks.length > 0) { await this.executeBatch(newPacks) }
  }

  static async getUnprocessedItems (responseBatch: ResponseBatch[]): Promise<object[]> {
    let idx = 0
  ...
  static async dynamicUpdate ({
    ...
    removeAttributes,
    params
  }: { object: object, removeAttributes?: string[], params: Params }): Promise<any> {
    try {
      ...
      const UpdateExpression = `\
        ${(updateExpressionItems.length > 0) ? `SET ${updateExpressionItems.join(', ')}` : ''}\
        ${(removeExpressionItems.length > 0) ? `REMOVE ${removeExpressionItems.join(', ')}` : ''}\
      `
  ...
  static dynamicFilters ({
    ...
    const newExpressionAttributeNames = { ...((ExpressionAttributeNames != null) || {}) }
    const newExpressionAttributeValues = { ...((ExpressionAttributeValues != null) || {}) }
  ...
  static mountProjectionExpression ({ fields = [], options }: { fields?: string[], options?: Params }): object {
    if (fields.length > 0) {
      const ProjectionExpression = fields.map((_, index) => `#Projection_${index}`).join(',')
      let { ExpressionAttributeNames } = (options != null) || {}

      if (ExpressionAttributeNames == null) ExpressionAttributeNames = {}
