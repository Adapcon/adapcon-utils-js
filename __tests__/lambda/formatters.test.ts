...
describe('invoke', () => {
  ...
  it('Should returns lambda response formatted without status code', () => {
    expect(lambdaResp(undefined ,'Created')).toEqual({ body: 'Created' });
  });
  ...
  it('Should returns lambda response error formatted without message', () => {
    expect(lambdaRespError({ statusCode: 500 })).toEqual({ statusCode: 500 });
  });

  it('Should returns lambda response error formatted without status and message', () => {
    expect(lambdaRespError({})).toEqual({ statusCode: 500 });
  });
});
